...
extern void emmc_get_life_time(struct mmc_card *card, u8 *ext_csd);
extern void emmc_life_time_dsm(struct mmc_card *card);
extern void emmc_async_resp_err_dsm(struct mmc_host *host,
	void (*mmc_wait_data_done)(struct mmc_request *));
extern void emmc_sync_resp_err_dsm(struct mmc_host *host, struct mmc_request *mrq,
	void (*mmc_wait_data_done)(struct mmc_request *));
extern void emmc_erase_err_dsm(struct mmc_card *card, struct mmc_command *cmd, int err);
extern void cache_flush_timeout_dsm(struct mmc_card *card);
extern void mmc_interrupt_hpi_failed_dsm(struct mmc_card *card, int err);
extern void cache_flush_err_dsm(struct mmc_card *card, int err);
extern void other_excep_event_dsm(u8 *ext_csd, struct mmc_card *card, struct request *req);
extern void emmc_read_write_err_dsm(struct mmc_card *card, int is_read, enum mmc_blk_status status);
extern void emmc_dsm_dclient_init(void);
