...
static int gpio_switch_back = HW_USB_VALUE_INVALID;
static int swstate_value = 0;
...
{
    char event_buf[HW_USB_EVENT_BUF_SIZE] = {0,};
    char *envp[] = {event_buf, NULL};
    int ret = 0;

    int pedestal_attach_flag = pedestal_attach;
    if (HW_USB_ONE == pedestal_attach_flag) {
    ...
    if (NULL == sdev.dev) {
        hwlog_info("[USB_DEBUG] %s: sdev.dev is NULL, do nothing!\n",__func__);
        return;
    }
    ret = kobject_uevent_env(&(sdev.dev->kobj), KOBJ_CHANGE, envp);
    hwlog_info("[USB_DEBUG] %s: sent uevent %s\n", __func__, envp[0]);

    if (ret < 0) {
        hwlog_err("[USB_DEBUG] %s: uevent sending failed!!! ret=%d\n",
                  __func__, ret);
    } else {
        hwlog_info("[USB_DEBUG] %s: uevent sending ok USB_CUSTOM_ACC5=custom_acc5_mode\n",
                   __func__);
    }
}
...
int get_swstate_value(void)
{
    ...
    hwlog_info("[USB_DEBUG] %s: swstate_value=%d\n", __func__, swstate_value);
    ...
    case USB_OFF:
        ...
        break;
    default:
        hwlog_err("[USB_DEBUG] %s: swstate_value[%d] is error!!!\n",
                  __func__, swstate_value);
        ret = FSA9685_OPEN;
        break;
    }
...
void switch_usb2_access_through_ap(void)
{
    ...
    if (ret < 0) {
        hwlog_err("[USB_DEBUG] %s: fsa9685_manual_sw error!!! ret = %d\n",
                  __func__, ret);
    }
}
...
{
    if (NULL == work) {
        /* Now beacuse the work ptr not be used, so we just print logs rather than return */
        hwlog_err("[USB_DEBUG] %s: work ptr is NULL!\n", __func__);
    }
...
static irqreturn_t int_gpio_irq_handler(int irq, void *dev_id)
{
    ...
    if (NULL == dev_id) {
        /* Now beacuse the work ptr not be used, so we just print logs rather than return */
        hwlog_err("[USB_DEBUG] %s: dev_id ptr is NULL!\n", __func__);
    }

    hwlog_info("[USB_DEBUG] %s: ------entry\n", __func__);
    if (gpio_switch_back >= 0) {
        int_gpio_value1 = gpio_get_value(gpio_switch_back);
        hwlog_info("[USB_DEBUG] %s: int_gpio_value1 = %d\n", __func__,
                    int_gpio_value1);
        if (GPIO_HI == int_gpio_value1) {
            schedule_work(&sdev.work);
        }
    } else {
        hwlog_err("[USB_DEBUG] %s: no gpio_switch_back\n", __func__);
    }
    hwlog_info("[USB_DEBUG] %s: ------end\n", __func__);
...
static ssize_t usb_state_show(struct device *dev, struct device_attribute *attr,
                              char *buf)
{
    struct switch_usb_dev *sdev = NULL;

    if ((NULL == dev) || (NULL == buf)) {
        hwlog_err("[USB_DEBUG] %s: dev or buf ptr is NULL!\n", __func__);
		return -EINVAL;
    }

    sdev = (struct switch_usb_dev *)dev_get_drvdata(dev);
    if (NULL == sdev) {
        hwlog_err("[USB_DEBUG] %s: sdev ptr is NULL!\n", __func__);
        return -EINVAL;
    }

	if (NULL == attr) {
        /* Now beacuse the work ptr not be used, so we just print logs rather than return */
        hwlog_err("[USB_DEBUG] %s: attr ptr is NULL!\n", __func__);
    }
			       ...
			       const char *buf, size_t size)
{
    ...
    if (switch_usb_set_state(sdev, state) < 0) {
        hwlog_info("[USB_DEBUG] %s: switch_usb_set_state err\n", __func__);
        return -EINVAL;
    }
...
{
    hwlog_info("[USB_DEBUG] %s: ------entry. state=%d\n", __func__, state);
    switch (state) {
    case USB_TO_AP:
    ...
    case USB_OFF:
        ...
        break;
    default:
        hwlog_err("[USB_DEBUG] %s: state[%d] is error!!!\n", __func__, state);
        break;
    }
    hwlog_info("[USB_DEBUG] %s: ------end.\n", __func__);
}
...
static void notify_switch_state(int state)
{
    atomic_notifier_call_chain(&p_switch_usb_info->charger_type_notifier_head,
                               state, NULL);
    hwlog_info("[USB_DEBUG] %s Notify usb switch state: %d\n", __func__, state);
}
...
static int switch_usb_set_state(struct switch_usb_dev *sdev, int state)
{
    ...
    if ((state < USB_TO_AP) || (state > USB_OFF)) {
        hwlog_err("[USB_DEBUG] %s: swstate[%d] is invalid!!!\n", __func__, state);
        ret = -EINVAL;
        return ret;
    }

    if (sdev->state == state) {
        hwlog_info("[USB_DEBUG] %s: swstate is not changed. sdev->state[%d] and state[%d]\n",
                   __func__, sdev->state, state);
        return -EINVAL;
    }

    set_reg_to_target_state(USB_OFF);
    msleep (HW_USB_SLEEP_1000);
...
static int create_switch_usb_class(void)
{
    ...
    if (NULL == switch_usb_class) {
        hwlog_info("[USB_DEBUG] %s: switch_usb_class=NULL and class_create will start.\n",
                   __func__);
        switch_usb_class = class_create(THIS_MODULE, "usbswitch");
        if (IS_ERR(switch_usb_class)) {
            hwlog_err("[USB_DEBUG] %s: class_create error!!! switch_usb_class=%p\n",
...
static int switch_usb_dev_register(struct switch_usb_dev *sdev)
{
    ...
    if (IS_ERR_OR_NULL(switch_usb_class)) {
        hwlog_info("[USB_DEBUG] %s: switch_usb_class is ERR or NULL, and create_switch_usb_class will start.",
                   __func__);
        ret = create_switch_usb_class();
        if (ret < 0) {
            hwlog_err("[USB_DEBUG] %s: create_switch_usb_class error!!! sdev->name=%s\n",
                       __func__, sdev->name);
            goto err_create_file;
        }
    }

    hwlog_info("[USB_DEBUG] %s: device_create will start.", __func__);
    sdev->dev = device_create(switch_usb_class, NULL, MKDEV(0, 0),
                              NULL, "usbsw");
    if (IS_ERR(sdev->dev)) {
        ret = PTR_ERR(sdev->dev);
        hwlog_err("[USB_DEBUG] %s: device_create error!!! sdev->name=%s\n",
                  __func__, sdev->name);
        goto err_create_file;
    }
...
{
    if (NULL == sdev) {
        /* Now sdev not be used, so we just print logs rather than return */
        hwlog_err("[USB_DEBUG] %s: sdev ptr is NULL!\n", __func__);
    }
...
static int usb_switch_probe(struct platform_device *pdev)
{
    ...
    if (NULL == pdev) {
        hwlog_err("[USB_DEBUG] %s: pdev or dev ptr is NULL!\n", __func__);
        return -EINVAL;
    }
    ...
    if (ret < 0) {
        hwlog_err("[USB_DEBUG] %s: switch_usb_dev_register error!!!\n", __func__);
        goto err_switch_usb_dev_register;
    }

    ret = device_create_file(sdev.dev, &dev_attr_swstate);
    if (ret < 0) {
        hwlog_err("[USB_DEBUG] %s: device_create_file error!!!\n", __func__);
        goto err_switch_usb_dev_register;
    }
    dev_set_drvdata(sdev.dev, &sdev);
    sdev.state = USB_TO_AP;

    gpio_switch_back = of_get_named_gpio(node, "gpio_switch_back", HW_USB_ZERO);
    hwlog_info("[USB_DEBUG] %s: gpio_switch_back=%d\n", __func__, gpio_switch_back);
    if (gpio_switch_back < 0) {
        hwlog_err("[USB_DEBUG] %s: of_get_named_gpio error!!! gpio_switch_back=%d",
                  __func__, gpio_switch_back);
        /* invalid value */
        sdev.irq1 = HW_USB_VALUE_INVALID;
    } else {
        sdev.irq1 = gpio_to_irq(gpio_switch_back);
        if (sdev.irq1 < 0) {
            ret = sdev.irq1;
            hwlog_err("[USB_DEBUG] %s: gpio_to_irq error!!! gpio_switch_back=%d\n",
                      __func__, gpio_switch_back);
        ...
        if (ret < 0) {
            hwlog_err("[USB_DEBUG] %s: gpio_request error!!! gpio_switch_back=%d\n",
                      __func__, gpio_switch_back);
        ...
        if (ret < 0) {
            hwlog_err("[USB_DEBUG] %s: gpio_direction_input error!!! gpio_switch_back=%d\n",
                      __func__, gpio_switch_back);
            goto err_set_gpio_input1;
        }

        ret = request_irq(sdev.irq1, int_gpio_irq_handler, IRQF_TRIGGER_RISING,
                          "usb_sw_interrupt1", &sdev);
        if (ret < 0) {
            hwlog_err("[USB_DEBUG] %s: request_irq error!!! sdev.irq1=%d\n",
                      __func__, sdev.irq1);
            goto err_request_irq1;
        }
    }

    hwlog_info("[USB_DEBUG] %s: ------OK.\n", __func__);
	return HW_USB_SUCCESS;
...
err_switch_usb_dev_register:
    hwlog_err("[USB_DEBUG] %s: ------FAIL!!! ret=%d\n", __func__, ret);
    return ret;
}
...
static int  usb_switch_remove(struct platform_device *pdev)
{
	if (NULL == pdev) {
        /* Now beacuse the work ptr not be used, so we just print logs rather than return */
        hwlog_err("[USB_DEBUG] %s: pdev ptr is NULL!\n", __func__);
    }
...
static int __init switch_usb_class_init(void)
{
    ...
    if (NULL == p_switch_usb_info) {
        ...
        if (NULL == sui) {
            hwlog_err("[USB_DEBUG] kzalloc failed!\n");
            return -ENOMEM;
        }
    ...
    hwlog_info("[USB_DEBUG] %s: ------end. ret=%d\n", __func__, ret);
    return ret;
}
