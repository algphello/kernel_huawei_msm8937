...
#define AW36413_DBG(fmt, args...) pr_info(fmt, ##args)
#else
#define AW36413_DBG(fmt, args...)
#endif

#define FLASH_CHIP_ID_REG 0x00
#define FLASH_CHIP_ID	0x36
...
/*
static struct msm_camera_i2c_reg_array aw36413_init_array[] = {
	{0x01, 0x00,0x00},//enable reg 0x01 ,mode bit 3:2 torch 10,flash 11,bit 1 led2,bit 0 led1
};
*/
static struct msm_camera_i2c_reg_array aw36413_low_array[] = {
	{0x05, 0x3F,0x00},//LED1 0x3f=186ma
	//{0x01, 0x09,0x00},
};

static struct msm_camera_i2c_reg_array aw36413_sub_low_array[] = {
...
static struct msm_camera_i2c_reg_array aw36413_high_array[] = {
	{0x03, 0x3F,0x00},//LED1 0x3F=750ma
	{0x08, 0x1A,0x00},//flash duration time 0x1A=600ms
	{0x01, 0x0D,0x00},
};

static struct msm_camera_i2c_reg_array aw36413_sub_high_array[] = {
	{0x03, 0x3F,0x00},//set reg 0x03 bit 7 is 0
	{0x05, 0x3F,0x00},//set reg 0x05 bit 7 is 0
...
static struct msm_camera_i2c_reg_array aw36413_torch_array[] = {
	{0x05, 0x3F, 0x00},//LED1 0x3f=186ma
	{0x01, 0x09, 0x00},
};

static struct msm_camera_i2c_reg_array aw36413_sub_torch_array[] = {
	{0x03, 0x3F,0x00},//set reg 0x03 bit 7 is 0
	{0x05, 0x3F,0x00},//set reg 0x05 bit 7 is 0
	{0x06, 0x22,0x00},//LED2 0x22=100ma
	{0x01, 0x0a,0x00},
...
static struct msm_camera_i2c_reg_setting aw36413_low_setting = {
	.reg_setting = aw36413_low_array,
	.size = ARRAY_SIZE(aw36413_low_array),
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
	.data_type = MSM_CAMERA_I2C_BYTE_DATA,
	.delay = 0,
};

static struct msm_camera_i2c_reg_setting aw36413_sub_low_setting = {
	.reg_setting = aw36413_sub_low_array,
	.size = ARRAY_SIZE(aw36413_sub_low_array),
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
	.data_type = MSM_CAMERA_I2C_BYTE_DATA,
	.delay = 0,
};

static struct msm_camera_i2c_reg_setting aw36413_high_setting = {
	.reg_setting = aw36413_high_array,
	.size = ARRAY_SIZE(aw36413_high_array),
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
	.data_type = MSM_CAMERA_I2C_BYTE_DATA,
	.delay = 0,
};

static struct msm_camera_i2c_reg_setting aw36413_sub_high_setting = {
	.reg_setting = aw36413_sub_high_array,
	.size = ARRAY_SIZE(aw36413_sub_high_array),
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
	.data_type = MSM_CAMERA_I2C_BYTE_DATA,
	.delay = 0,
};

static struct msm_camera_i2c_reg_setting aw36413_torch_setting = {
	.reg_setting = aw36413_torch_array,
	.size = ARRAY_SIZE(aw36413_torch_array),
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
	.data_type = MSM_CAMERA_I2C_BYTE_DATA,
	.delay = 0,
};

static struct msm_camera_i2c_reg_setting aw36413_sub_torch_setting = {
	.reg_setting = aw36413_sub_torch_array,
	.size = ARRAY_SIZE(aw36413_sub_torch_array),
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
	.data_type = MSM_CAMERA_I2C_BYTE_DATA,
	.delay = 0,
};

static int32_t hw_flash_i2c_aw36413_check_flag(struct msm_flash_ctrl_t *flash_ctrl)
{
	int rc = 0;
		...
		struct msm_flash_cfg_data_t *flash_data)
{
	...
	if (NULL != flash_ctrl->power_info.gpio_conf && NULL != flash_ctrl->power_info.gpio_conf->gpio_num_info) {
		...
		hw_flash_i2c_aw36413_check_flag(flash_ctrl);
	} else {
...
int32_t hw_flash_i2c_aw36413_release(struct msm_flash_ctrl_t *flash_ctrl)
{
	...
	if (NULL != flash_ctrl->power_info.gpio_conf && NULL != flash_ctrl->power_info.gpio_conf->gpio_num_info) {
		...
		if(flash_ctrl->flash_i2c_client.i2c_func_tbl){
			...
			AW36413_DBG("%s:reg_val:[0x%x],(reg_val&0x03):[0x%x]\n",__func__, reg_val,(reg_val&0x03));
	...
	if (NULL != flash_ctrl->power_info.gpio_conf && NULL != flash_ctrl->power_info.gpio_conf->gpio_num_info) {
		...
		if(flash_ctrl->flash_i2c_client.i2c_func_tbl){
			if (NULL != flash_data && flash_data->flash_current[0] > TORCH_MIN_CURRENT
				...
				reg_data = (flash_data->flash_current[0]*100 - 255)/291;//ITORCH(mA)=(Brightness Code*2.91mA)+2.55mA
			}

			aw36413_low_setting.reg_setting[0].reg_data = reg_data;
			...
			AW36413_DBG("%s:reg_val:[0x%x],(reg_val&0x0F)|0x09:[0x%x]\n",__func__, reg_val,(reg_val&0x0F)|0x09);
	...
	if (NULL != flash_ctrl->power_info.gpio_conf && NULL != flash_ctrl->power_info.gpio_conf->gpio_num_info) {
		...
		if(flash_ctrl->flash_i2c_client.i2c_func_tbl){
			...
			if (NULL != flash_data) {
				reg_data = (flash_data->flash_current[0]*100 - 255)/291;//ITORCH(mA)=(Brightness Code*2.91mA)+2.55mA
				if (reg_data > 127){
					reg_data = 0x0c;//0x0c = 40ma
				}
			} else {
				reg_data = 0x21;//0x21 = 98ma
			}
			AW36413_DBG("reg_data:[%d]\n",reg_data);
			...
			AW36413_DBG("%s:reg_val:[0x%x],(reg_val&0x0F)|0x0a:[0x%x]\n",__func__, reg_val,(reg_val&0x0F)|0x0a);
			rc = flash_ctrl->flash_i2c_client.i2c_func_tbl->i2c_write(
		...
		struct msm_flash_cfg_data_t *flash_data)
{
	...
	if (NULL != flash_ctrl->power_info.gpio_conf && NULL != flash_ctrl->power_info.gpio_conf->gpio_num_info) {
		if(flash_ctrl->flash_i2c_client.i2c_func_tbl){
			...
			AW36413_DBG("%s:reg_val:[0x%x],reg_val&0x0a:[0x%x]\n",__func__, reg_val,reg_val&0x0a);
			rc = flash_ctrl->flash_i2c_client.i2c_func_tbl->i2c_write(
...
/****************************************************************************
* FunctionName: msm_flash_aw36413_match_id;
* Description :read id and compared with FLASH_CHIP_ID;
***************************************************************************/
int32_t hw_flash_i2c_aw36413_match_id(struct msm_flash_ctrl_t *flash_ctrl,struct i2c_flash_info_t*flash_info)
{
	int32_t rc = 0;
	int32_t i = 0;
	uint16_t id_val = 0;
	...
	if (NULL != flash_ctrl->power_info.gpio_conf && NULL != flash_ctrl->power_info.gpio_conf->gpio_num_info) {
		...
		if (flash_ctrl->flash_i2c_client.i2c_func_tbl) {
			for(i = 0; i < 3; i++){
				...
				pr_info("%s, aw36413 id:0x%x, rc = %d\n", __func__,id_val,rc );
