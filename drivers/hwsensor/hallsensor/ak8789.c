...
#define AK8789_GPIO_IS_HIGH     1
#define AK8789_GPIO_IS_LOW      0
#define AK8789_GPIO_NUM_1       1
#define AK8789_GPIO_NUM_2       2
#define AK8789_GPIO_NUM_4       4
#define AK8789_BUF_LEN          10
#define AK8789_VAL_ZERO         0
#define AK8789_VAL_ONE          1
#define AK8789_VAL_0X10         (0x10)
#define AK8789_VAL_0X20         (0x20)
#define AK8789_ATOMIC_READ_ZERO 0


#define AK8789_VAL_1            1
#define AK8789_VAL_2            2
#define AK8789_VAL_4            4
#define AK8789_VAL_5            5

#define AK8789_DELAY_VAL        200
...
typedef enum hall_used_type{
	...
	ONE_POLE_ONE_PIN = AK8789_VAL_ZERO,
	ONE_POLE = AK8789_VAL_1,
	TWO_POLE = AK8789_VAL_2,
	FOUR_POLE = AK8789_VAL_4,
	/*usd for camare hall*/
	ONE_POLE_FOR_CAMARE = AK8789_VAL_5,
} hall_used_type_t;
...
static int hall_irq_level_set(gpio_data_t *gpio_ptr)
{
	...
	if(AK8789_GPIO_IS_HIGH == gpio_val)
	{
	...
	else if (AK8789_GPIO_IS_LOW == gpio_val)
	{
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	val = simple_strtoul(buf, NULL, AK8789_BUF_LEN);

	AK8789_FLOWMSG("enable_status: %d; enable value %lu", atomic_read(&hall_enable_status), val);
	if ((val == AK8789_VAL_ONE) && (atomic_read(&hall_enable_status) == AK8789_ATOMIC_READ_ZERO)){
		/*enable the hall device*/
		atomic_set(&hall_enable_status, AK8789_VAL_ONE);
		...
		for (i = 0;  i < hw_hall_dev.gpio_nums;  i++)
		{
			...
			enable_irq(irq);
			if((IRQF_TRIGGER_NONE == wake_flags)||(IRQF_NO_SUSPEND == wake_flags))
			{
				irq_set_irq_wake(irq , AK8789_VAL_ONE);
			}
			gpio_ptr++;
			AK8789_FLOWMSG("irq enable : %d; wake_flags: 0x%lx;", irq,  wake_flags);
		}
	}else if ((val == AK8789_VAL_ZERO) && (atomic_read(&hall_enable_status) == AK8789_VAL_ONE)){
		/*disable the hall devices*/
		atomic_set(&hall_enable_status, AK8789_VAL_ZERO);
		gpio_ptr = hw_hall_dev.gpio_data;
		for (i = 0;  i < hw_hall_dev.gpio_nums;  i++)
		{
			irq =  gpio_to_irq(gpio_ptr->gpio);
			...
			{
				irq_set_irq_wake(irq , AK8789_VAL_ZERO);
			}
			disable_irq(irq);
			gpio_ptr++;
				...
				struct device_attribute *attr, char *buf)
{
	if (NULL == buf)
	{
		AK8789_FLOWMSG("ak8789_show_irq_count: buf ptr is NULL!");
        return AK8789_FUN_RET_FAIL;
	}
				...
				struct device_attribute *attr, char *buf)
{
	if (NULL == buf)
	{
		AK8789_FLOWMSG("ak8789_show_support_camera_hall_status: buf ptr is NULL!");
        return AK8789_FUN_RET_FAIL;
	}
...
static ssize_t ak8789_show_get_camera_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if ((NULL == buf) || (NULL == hw_hall_dev.gpio_data))
	{
...
static ssize_t ak8789_show_mmi_camera_hall_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if ((NULL == buf) || (NULL == hw_hall_dev.gpio_data))
	{
		AK8789_FLOWMSG("ak8789_show_mmi_camera_hall_status: buf or gpio_data is NULL!");
        return AK8789_FUN_RET_FAIL;
	}	
	gpio_ptr = hw_hall_dev.gpio_data;
	for ( i = 0; i < hw_hall_dev.gpio_nums; i++){
...
int check_support_camera_hall_status(struct hall_dev *hall_dev)
{
	...
	if ((NULL == hall_dev) || (NULL == hall_dev.gpio_data))
	{
		AK8789_FLOWMSG("check_support_camera_hall_status: hall_dev or gpio_data is NULL!");
        return AK8789_FUN_RET_FAIL;
	}
	gpio_ptr = hall_dev->gpio_data;
	gpio_nums = hall_dev->gpio_nums;
...
void hall_work_func(struct work_struct *work)
{
	...
	if (NULL == hw_hall_dev.hw_input_camera_hall)
	{
		AK8789_FLOWMSG("hall_work_func: hw_input_camera_hall is NULL!");
        return AK8789_FUN_RET_FAIL;
	}

	if(AK8789_VAL_ONE == atomic_read(&camera_hall_need_report))
	{
		atomic_set(&camera_hall_need_report, AK8789_VAL_ZERO);
		value = query_hall_event();
		if(value & HALL_VALUE_CAMARA_SOUTH)
		{
			/*report key event down for camera Rotation*/
			input_report_key(hw_hall_dev.hw_input_camera_hall, BTN_TRIGGER_HAPPY, AK8789_VAL_ONE);
			input_sync(hw_hall_dev.hw_input_camera_hall);
			/*report key event up for camera Rotation*/
			input_report_key(hw_hall_dev.hw_input_camera_hall, BTN_TRIGGER_HAPPY, AK8789_VAL_ZERO);
			input_sync(hw_hall_dev.hw_input_camera_hall);
	...
	value = query_hall_event();
	if((camera_hall_support_is_true == true) && ((value == AK8789_VAL_0X10) || (value == AK8789_VAL_0X20)))
		report_overturn_num += AK8789_VAL_ONE; 
#ifdef CONFIG_LOG_JANK
	if(!value)
	{
		LOG_JANK_D(JLID_COVER_SENSOR_OPEN, "%s", "JL_COVER_SENSOR_OPEN");
...
irqreturn_t hall_event_isr(int irq, void *dev)
{
	...
	if(is_camera_hall_irq_occur( irq, &hw_hall_dev))
	{
		...
		{
			atomic_set(&camera_hall_need_report, AK8789_VAL_ONE);
...
static int hall_parse_dt(struct device *dev)
{
	...
	switch(used_type)
	{
		...
		case ONE_POLE_ONE_PIN:
			...
			hw_hall_dev.gpio_nums = AK8789_GPIO_NUM_1;
			break;
		case FOUR_POLE:
			which_pole = four_pole_config;
			hw_hall_dev.gpio_nums = AK8789_GPIO_NUM_4;
			break;
		case TWO_POLE:
			which_pole = two_pole_config;
			hw_hall_dev.gpio_nums = AK8789_GPIO_NUM_2;
			break;
		case ONE_POLE:
