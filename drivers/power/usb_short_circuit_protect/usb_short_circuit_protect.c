...
static void uscp_external_power_changed(struct power_supply *psy)
{
    ...
    usb_present = get_propety_int(g_di->usb_psy, POWER_SUPPLY_PROP_PRESENT);
	if(usb_present^g_di->usb_present){
		g_di->usb_present = usb_present;
    	type = get_propety_int(g_di->usb_psy, POWER_SUPPLY_PROP_TYPE);
		pr_info("uscp_external_power_changed type=%d usb_present=%d\n",type,usb_present);
		charge_type_handler(g_di,type);
	}
}
