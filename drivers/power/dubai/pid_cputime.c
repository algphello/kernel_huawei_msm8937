		...
		unsigned char *value)
{
	...
	if (strlen(pid_entry->name) > 0) {
		stat.cmdline = true;
		dubai_copy_name(stat.name, pid_entry->name);
	} else {
		stat.cmdline = false;
		dubai_copy_name(stat.name, pid_entry->comm);
	}
...
int dubai_update_pid_cputime(void)
{
	...
	do_each_thread(temp, task) {
		...
		if (pid_entry == NULL) {
			rcu_read_unlock();
			DUBAI_LOGE("Failed to find the pid_entry for pid %d", task->tgid);
			return -ENOMEM;
		}
		...
		unsigned long cmd, void *v)
{
	...
	if (pid == task->pid || dubai_task_alive(leader)) {
		pid_entry = dubai_find_or_register_pid(task);
		if (pid_entry != NULL && got_cmdline)
			dubai_copy_name(pid_entry->name, cmdline);
	} else {
		pid_entry = dubai_find_pid_entry(pid);
	}
