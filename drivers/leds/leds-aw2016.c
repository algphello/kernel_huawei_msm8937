...
#define LED_DEBUG_AW2016				0	//when debugging  the led, we set this macro to 1
#define AW2016_DELAY_2MS				2
#define INPUT_ITEMS_NUM					2	// it should have 2 input items when we need writing data to reg.
...
/*
 * function: write control data to the regs of aw2016 with i2c bus.
 * aw2016_led:used to obtain the i2c clinet.
 * reg: register address.
 * val: the controling data.
 */
static int aw2016_write(struct aw2016_led *led, u8 reg, u8 val)
{
	...
	if((NULL == led) || (val < 0)) {
		pr_err("[aw2016]%s: the function attr of aw2016_write has error.\n",__func__);
		return ret;
	}
	...
	do {
		...
		retry_times ++;
		if(retry_times == 5) {
			pr_err("[aw2016]%s: aw2016_write failed!!! led:%s, reg:%d. val:%d.\n",__func__,led->cdev.name, reg, val);
			break;
		}
	}while (ret < 0);
...
/*
 * function: read data from the register in aw2016 with i2c bus.
 * aw2016_led:used to obtain the i2c clinet.
 * reg: register address.
 * val: register data.
 */
static int aw2016_read(struct aw2016_led *led, u8 reg, u8 *val)
{
	...
	if (ret < 0) {
		pr_err("[aw2016]%s: aw2016_read failed!!! led:%s, reg:%d. \n",__func__,led->cdev.name,reg);
		return ret;
	}
...
static void aw2016_brightness_work(struct work_struct *work)
{
    u8 val = 0;
	if(NULL == work) {
		pr_err("[aw2016]%s: set brightness failed! the attr work_struct is NULL .\n",__func__);
		return;
	}
	struct aw2016_led *led = container_of(work, struct aw2016_led,
					brightness_work);
	if(NULL == led) {
		pr_err("[aw2016]%s: container_of failed! plealse check the cdev in the struct aw2016_led.\n",__func__);
		return;
	}

#if	LED_DEBUG_AW2016
	pr_err("[aw2016]%s:%s leds set brightness =%d.\n",__func__,led->cdev.name,led->cdev.brightness);
#endif
...
static void aw2016_led_blink_set(struct aw2016_led *led, unsigned long blinking)
{
	u8 val = 0;

	if(NULL == led) {
		pr_err("[aw2016]%s: led_blink_set failed! led is NULL .\n",__func__);
		return;
	}
	/* enable aw2016 if disabled */
	aw2016_read(led, AW2016_REG_GCR1, &val);
...
#if LED_DEBUG_AW2016
	pr_err("[aw2016]%s:%s leds set blink on=%lu,off=%lu.\n",__func__,led->cdev.name, led->pdata->hold_time_ms, led->pdata->off_time_ms);
#endif
...
{
	if((NULL == cdev) || (brightness < 0)) {
		pr_err("[aw2016] %s the aw2016_set_brightness's attr has error, please check it out \n", __func__);
		return;
	}
			     ...
			     const char *buf, size_t len)
{
	...
	if((NULL == dev) || (NULL == buf) ||(len <= 0)) {
		pr_err("[aw2016] %s funtion aw2016_store_blink's attr has error, please check it out \n", __func__);
		return -EINVAL;
	}

	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	if(	NULL == led_cdev) {
		pr_err("[aw2016] %s get the driver data failed!\n", __func__);
		return -EINVAL;
	}
	...
	ret = kstrtoul(buf, 10, &blinking); //the number base to use is 10.
	if (ret) {
		pr_err("[aw2016] %s we try to transfer the string to unsigned long, but failed! possibly the illegal buf.\n", __func__);
		return ret;
	}
	mutex_lock(&led->pdata->led->lock);
	aw2016_led_blink_set(led, blinking);
	mutex_unlock(&led->pdata->led->lock);
				...
				struct device_attribute *attr, char *buf)
{
	...
	if((NULL == dev) || (NULL == buf)) {
		pr_err("[aw2016] %s funtion aw2016_led_time_show's attr has error, please check it out \n", __func__);
		return ret;
	}
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	if(	NULL == led_cdev) {
			     ...
			     const char *buf, size_t len)
{
	...
	if((NULL == dev) || (NULL == buf) || (len <= 0)) {
		pr_err("[aw2016] %s funtion aw2016_led_time_store's attr has error, please check it out \n", __func__);
		return -EINVAL;
	}
...
static ssize_t aw2016_reg_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	unsigned char i = 0x00, reg_val = 0x00;
	ssize_t len = 0;

	if((NULL == dev) || (NULL == buf)) {
		pr_err("[aw2016] %s funtion aw2016_reg_show's attr has error, please check it out \n", __func__);
		return -EINVAL;
	}
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	if(	NULL == led_cdev) {
...
static ssize_t aw2016_reg_store(struct device *dev,
				...
				const char *buf, size_t len)
{
	unsigned int databuf[INPUT_ITEMS_NUM];

	if((NULL == dev) || (NULL == buf) || (len <= 0)) {
		pr_err("[aw2016] %s funtion aw2016_reg_store's attr has error, please check it out \n", __func__);
		return -EINVAL;
	}
	...
	if(INPUT_ITEMS_NUM == sscanf(buf, "%x %x", &databuf[0], &databuf[1])) {    // it should have 2 input items successfully matched and assigned.
...
/*
 * create the blink、led_time、reg node in sysfs (sys/class/leds/...)
 * we seldom use the led_time and reg node
 */
...
static int aw2016_check_chipid(struct aw2016_led *led)
{
	...
	if(NULL == led) {
		pr_err("[aw2016] %s funtion aw2016_check_chipid's attr has error, please check it out \n", __func__);
		return -EINVAL;
	}

	for(cnt = 5; cnt > 0; cnt --)   // It return error if we read 5 times unsuccessfully.
	{
				...
				int parsed_leds)
{
	int i;
	if(NULL == led_array) {
		pr_err("[aw2016] %s funtion aw2016_led_err_handle's attr has error, please check it out \n", __func__);
		return -EINVAL;
	}
	/*
...
{
	struct aw2016_led *led = NULL;
	struct device_node *temp = NULL;
	struct aw2016_platform_data *pdata = NULL;
	int rc = 0, parsed_leds = 0;

	if((NULL == led_array) || (NULL == node) ) {
		pr_err("[aw2016] %s funtion aw2016_led_parse_child_node's attr has error, please check it out \n", __func__);
		return -EINVAL;
	}
	...
	for_each_child_of_node(node, temp) {
		...
		if (!pdata) {
			...
			"[aw2016] %s Failed to allocate memory for aw2016_platform_data\n", __func__);
			goto free_err;
		}
		pdata->led = led_array;
		led->pdata = pdata;
		...
		if (rc < 0) {
			dev_err(&led->client->dev,
			"[aw2016] %s: reading the property of [aw2016,name] failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc < 0) {
			dev_err(&led->client->dev,
			"[aw2016] %s: reading the property of [aw2016,id] failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc < 0) {
			dev_err(&led->client->dev,
			"[aw2016] %s: reading the property of [aw2016,imax] failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc < 0) {
			dev_err(&led->client->dev,
			"[aw2016] %s: reading the property of [aw2016,led-current] failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc < 0) {
			dev_err(&led->client->dev,
			"[aw2016] %s: reading the property of [aw2016,max-brightness] failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc < 0) {
			dev_err(&led->client->dev,
			"[aw2016] %s: reading the property of [aw2016,rise-time-ms] failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc < 0) {
			dev_err(&led->client->dev,
			"[aw2016] %s: reading the property of [aw2016,hold-time-ms] failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc < 0) {
			dev_err(&led->client->dev,
			"[aw2016] %s: reading the property of [aw2016,fall-time-ms] failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc < 0) {
			dev_err(&led->client->dev,
			"[aw2016] %s: reading the property of [aw2016,off-time-ms] failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc) {
			dev_err(&led->client->dev,
				"[aw2016] %s:led_classdev_register failed, rc = %d\n", __func__, rc);
			goto free_pdata;
		}
		...
		if (rc) {
			dev_err(&led->client->dev,
				"[aw2016] %s:sysfs_create_group failed, rc = %d\n", __func__, rc);
			goto free_class;
		}
		parsed_leds++;
	}
...
{
	struct aw2016_led *led_array = NULL;
	struct device_node *node = NULL;
	int ret = -EINVAL, num_leds = 0;

	if (NULL == client) {
		pr_err("[aw2016] %s funtion aw2016_led_remove's attr client is NULL, please check it out \n", __func__);
		return ret;
	}
	...
	if (NULL == node) {
		dev_err(&client->dev,
		"[aw2016] %s: led probe failed; i2c_client->dev.of_node is NULL.\n",__func__);
		return -EINVAL;
	}

	pr_info("[aw2016]%s: led probe start.\n",__func__);

	num_leds = of_get_child_count(node);
	if (!num_leds) {
		dev_err(&client->dev,
		"[aw2016] %s: led probe failed; do not have led node in the device tree.\n",__func__);
		return -EINVAL;
	}
	...
	if (!led_array) {
		dev_err(&client->dev, "[aw2016] %s allocate space for aw2016_led dev failed.\n",__func__);
		return -ENOMEM;
	}
	...
	if (ret) {
		dev_err(&client->dev, "[aw2016] %s parsed node failed\n", __func__);
		goto free_led_arry;
	}
	...
	if (ret) {
		dev_err(&client->dev, "[aw2016] %s aw2016_check_chipid failed.\n",__func__);
		goto fail_parsed_node;
	}

	pr_info("[aw2016]%s: led probe success.\n",__func__);
	return 0;
...
static int aw2016_led_remove(struct i2c_client *client)
{
	...
	if(	NULL == led_array) {
		pr_err("[aw2016] %s get the client data failed!\n", __func__);
		return -EINVAL;
	}
...
static int aw2016_led_shutdown(struct i2c_client *client)
{
	if (NULL == client) {
		pr_err("[aw2016] %s funtion aw2016_led_remove's attr client is NULL, please check it out \n", __func__);
		return -EINVAL;
