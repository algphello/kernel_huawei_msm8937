...
void tfa_set_query_info(struct tfa_device *tfa)
{
	...
	case 0x72:
		...
		pr_info("%s tfa spk num is 0x%x\n",__func__, tfa->spkr_count);
		break;
	case 0x74:
		/* tfa9874 */
		tfa->supportDrc = supportYes;
...
enum Tfa98xx_Error tfa98xx_set_mtp_cal(struct tfa_device *tfa, uint16_t value)
{
	int loop, status;
	enum Tfa98xx_Error error;

	pr_err("tfa tfa98xx_set_mtp_cal\n");
	/* Assure there is a clock to MTP (enable it when neccesery) */
	error = tfa98xx_set_mtp_clk(tfa, 1);
	if (error) {
		return error;
	...
	tfa98xx_key2(tfa, 0); /* unlock */
	TFA_WRITE_REG(tfa, R25C, value); 	/* write to i2c shadow reg */
	/* CIMTP=1 start copying all the data from i2c regs_mtp to mtp*/
	TFA_SET_BF(tfa, CIMTP, 1);
	/* no check for MTPBUSY here, i2c delay assumed to be enough */
	tfa98xx_key2(tfa, 1); /* lock */
...
enum Tfa98xx_Error tfaRunStartup(struct tfa_device *tfa, int profile)
{
	...
	err = tfa_dev_set_state(tfa, TFA_STATE_INIT_CF);
    if (err)
        return err;
