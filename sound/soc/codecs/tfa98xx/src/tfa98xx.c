...
int tfa98xx_detected = 0;
struct tfa98xx_rate {
	unsigned int rate;
	unsigned int fssel;
};

#ifdef TFA98XX_SUPPORT_CALIBRATION_DEVICE_FTR
#define CALIBRATION_DEVICE_NAME "tfa98xx_calibrate"
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("rpc", S_IRUGO|S_IWUSR|S_IWGRP, tfa98xx->dbg_dir,
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	pr_info("%s tfa device is 0x%x\n",__func__,tfa98xx->tfa->rev);
	...
	/* this part is not necessary for tfa9872, and may need to adapt for other
	 * product */
	if (tfa98xx->tfa->tfa_family == 2) {
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa98xx_tfa_start(tfa98xx, tfa98xx->profile, tfa98xx->vstep);
...
#else
	pr_debug("remove Preload setting using internal clock on TFA2\n");
#endif
...
static void tfa98xx_interrupt(struct work_struct *work)
{
    ...
    /* cleaning all interruption flag. */
	tfa_irq_clear(tfa98xx->tfa, tfa9912_irq_all);
...
static int tfa98xx_adsp_cmd(struct tfa98xx *tfa98xx, int cmd_id, uint8_t *buf, ssize_t buf_sz)
{
	int ret = 0;

	if (buf == NULL || tfa98xx == NULL) {
		return -1;
	}
...
static int tfa98xx_adsp_send_calibration_values(struct tfa98xx *tfa98xx)
{
	int ret = 0, nr = 0;
	unsigned char bytes[3*3] = {0};
	int value, dsp_cal_value_left=0, dsp_cal_value_right=0;

	if (tfa98xx == NULL) {
		pr_err("%s error, tfa98xx is NULL\n",__func__);
		return -EFAULT;
	}

	pr_info("%s entry\n",__func__);
	/* If calibration is set to once we load from MTP, else send zero's */
	if (TFA_GET_BF(tfa98xx->tfa, MTPEX) == 1)
	{
		value = tfa_dev_mtp_get(tfa98xx->tfa, TFA_MTP_RE25);
		if (value > 4000 && value < 9000) {
			...
			ret = send_tfa_cal_in_band(bytes, sizeof(bytes));
		}
	}
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			tfa98xx->pstream = 1;
			/* Send calibrate values to DSP */
			if (0x72 == (tfa->rev & 0xff)) {
				pr_info("%s send mtp to algo\n",__func__);
				ret = tfa98xx_adsp_send_calibration_values(tfa98xx);
				if (ret) {
					pr_err("%s send mtp to algo error, ret:%d\n",__func__, ret);
				}
			}
...
static int tfa98xx_calibration_open(struct inode *inode, struct file *file)
{
	...
	if (file == NULL) {
		pr_err("%s error, ptr is NULL\n",__func__);
		return -EINVAL;
	}

	tfa98xx = container_of(file->private_data, struct tfa98xx, calibrate_device);

	pr_info("%s: entry  tfa98xx=%p\n", __func__, tfa98xx);
	if (tfa98xx) {
...
static ssize_t tfa98xx_calibration_show(struct file *file, char __user *user_buf,
                                                size_t count,loff_t *ppos)
{
	...
	if (file == NULL || user_buf == NULL) {
		pr_err("%s	ptr is NULL.\n", __func__);
		return -EINVAL;
	}
	tfa98xx = file->private_data;
	...
	if (0x12 == (tfa->rev & 0xff)) {
		...
		ret = tfaRunWaitCalibration(tfa, &calibrateDone);
		if (ret){
			pr_err("run API tfaRunWaitCalibration() returned error(%d)  calibrateDone=%d\n", ret, calibrateDone);
			ret = -ret;
			goto err;
		}

		if (1 != calibrateDone){
			pr_err("run API tfaRunWaitCalibration() ret:(%d)  calibrateDone=%d\n", ret, calibrateDone);
			ret = Tfa98xx_Error_Fail;
			goto err;
		}

		/* get speaker count */
		ret = tfa_supported_speakers(tfa, &calibratedata.spk_count);
		if (ret){
			pr_err("run API tfa98xx_supported_speakers() returned error(%d)\n", ret);
			goto err;
		}

		/* get impedance data.*/
		ret = tfa_dsp_get_calibration_impedance(tfa);
		if (ret){
			pr_err("run API tfa_dsp_get_calibration_impedance() returned error(%d)\n", ret);
			goto err;
		}else{
			int i = 0;
			for(i=0; i<calibratedata.spk_count; i++){
				calibratedata.impedance[i] = tfa_get_calibration_info(tfa, i);
			}
		}
		...
		ret = tfa_dsp_cmd_id_write_read(tfa, MODULE_SPEAKERBOOST, SB_PARAM_GET_LSMODEL,
											TFA1_SPEAKERPARAMETER_LENGTH, buff);
		if(ret){
			pr_err("run API tfa_dsp_cmd_id_write_read() returned error(%d)\n", ret);
			goto err;
		}else{
			tfa98xx_convert_bytes2data(TFA1_SPEAKERPARAMETER_LENGTH, buff, lsmodel_data);
			calibratedata.Fres	  = lsmodel_data[135];
			calibratedata.Qfactory  = lsmodel_data[137];
			calibratedata.Tcof	  = lsmodel_data[140];
		}
	} else if (0x72 == (tfa->rev & 0xff)) {
		uint8_t *buffer;
		int count = 512;

		pr_info(" %s TFA9872 will show Fres/Q/Tcof \n",__func__);
		...
		calibratedata.impedance[0]= tfa_dev_mtp_get(tfa, TFA_MTP_RE25);

		ret = tfa98xx_adsp_cmd(tfa98xx, 0x00808b, buffer, count);

		calibratedata.ReT = (buffer[5*3 + 0] <<16) + (buffer[5*3 + 1] << 8) + buffer[5*3 + 2];
		calibratedata.Temp = (buffer[9*3 + 0] <<16) + (buffer[9*3 + 1] << 8) + buffer[9*3 + 2];
		calibratedata.Fres = (buffer[41*3 + 0] <<16) + (buffer[41*3 + 1] << 8) + buffer[41*3 + 2];

		pr_info(" %s cal show Re0: 0x%x ReT: 0x%x Temp: 0x%x FRes: 0x%x IC temp %d\n", __func__, calibratedata.impedance[0],
			calibratedata.ReT, calibratedata.Temp, calibratedata.Fres,TFA_GET_BF(tfa98xx->tfa, TEMPS));
...
err:
	...
	ret = copy_to_user(user_buf, &calibratedata, sizeof(struct tfa98xx_calibratedata));
	if (ret)
		return -ret;
	else
		return count;
...
static ssize_t tfa98xx_do_calibration(struct file *file, const char __user *user_buf,
											size_t count, loff_t *ppos)
{
	enum Tfa98xx_Error ret = Tfa98xx_Error_Ok;
	struct i2c_client* i2c = NULL;
	struct tfa98xx *tfa98xx;
	struct tfa_device *tfa;
	char kernelbuf[MAX_BUFFER_LENGTH] = {0};
	int ready = 0;
	...
	pr_info("%s: Device 0x%x is detected.\n", __func__,(tfa->rev & 0xff));

	if (0x12 == (tfa->rev & 0xff)) {
		mutex_lock(&tfa98xx->dsp_lock);

		memset(kernelbuf, 0x00, MAX_BUFFER_LENGTH);
		if (count >= MAX_BUFFER_LENGTH) {
			ret = copy_from_user(kernelbuf, user_buf, MAX_BUFFER_LENGTH);
		} else {
			ret = copy_from_user(kernelbuf, user_buf, count);
		}

		if (ret) {
			pr_err("%s run API copy_from_user() returned error(%d)\n", ret);
			goto err;
		}
		...
		ret = tfaRunStartup(tfa, tfa->profile);
		if (ret) {
			pr_err("run API tfaRunStartup() returned error(%d)\n", ret);
			goto err;
		}

		ret = tfaRunColdboot(tfa, 1);
		if (ret) {
		...
		if (0 == strncmp(kernelbuf, "once", strlen("once"))) {
			ret = tfa98xx_set_mtp(tfa, 1, (1<<TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_POS));
		} else {
			ret = tfa98xx_set_mtp(tfa, 0, (1<<TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_POS));
		}
		if (ret) {
		...
		ret = tfaRunSpeakerCalibration(tfa);
		if (ret) {
			pr_err("run API tfaRunSpeakerCalibration() returned error %d\n", ret);
		} else {
			pr_info("%s Device 0x%x new impedance is %d mohm\n", __func__, i2c->addr, tfa_get_calibration_info(tfa, 0));

			ret = tfa_set_filters(tfa, tfa->profile);
			if (ret) {
				pr_err("run API tfa_set_filters() returned error %d\n", ret);
			}

			//tfa_set_swprof(tfa, tfa->profile);
			ret = tfa98xx_set_mute(tfa, Tfa98xx_Mute_Off);
			if (ret) {
		...
		while ((tfa98xx->dsp_init != TFA98XX_DSP_INIT_DONE) &&
			(i < CAL_WAIT_TIME_MAX))
		{
			msleep(100);
			i++;
		...
		if( CAL_WAIT_TIME_MAX == i ) {
			pr_err("%s TFA9872 is not ready\n",__func__);
			goto clib_fail;
		}

		pr_info(" %s start calibrating \n",__func__);

		ret = tfa98xx_adsp_cmd(tfa98xx, 0x00808d, buffer, count);
		if (ret) {
			pr_err("[0x%x] dsp_msSg_read error: %d\n", i2c->addr, ret);
			kfree(buffer);
			return -EFAULT;
		}

		ret = tfa98xx_adsp_cmd(tfa98xx, 0x048105, buffer, count);
		if (ret) {
			pr_err("[0x%x] dsp_msSg_read error: %d\n", i2c->addr, ret);
			kfree(buffer);
			return -EFAULT;
		...
		if( CAL_WAIT_TIME_MAX == i ) {
			pr_err("%s calibration timeout\n",__func__);
			goto clib_fail;
		}

		/* Read calibration impendance */
		ret = tfa98xx_adsp_cmd(tfa98xx, 0x048185, buffer, count);
		if (ret) {
			pr_err("[0x%x] dsp_msg_read error: %d\n", i2c->addr, ret);
			kfree(buffer);
			return -EFAULT;
		...
		cal_value = (buffer[0] << 16) | (buffer[1] << 8) | (buffer[2]);

		/* Set MTPEX to indicate calibration is done! */
		ret = tfa_dev_mtp_set(tfa, TFA_MTP_OTC, 1);
		if (ret) {
			pr_err("[0x%x] tfa_dev_mtp_set error: %d\n", i2c->addr, ret);
			kfree(buffer);
			return -EFAULT;
		}

		/* Write calibration value to MTP */
		tfa98xx_set_mtp_cal(tfa,(uint16_t)(cal_value*1000/0x10000));
		if (ret) {
			pr_err("[0x%x] tfa_dev_mtp_set error: %d\n", i2c->addr, ret);
			kfree(buffer);
			return -EFAULT;
		}
		ret = tfa_dev_mtp_set(tfa, TFA_MTP_EX, 2);
		if (ret) {
			pr_err("[0x%x] tfa_dev_mtp_set error: %d\n", i2c->addr, ret);
			kfree(buffer);
			return -EFAULT;
		}
		pr_info("%s Calibration impendance 0x%x\n", __func__,tfa_dev_mtp_get(tfa, TFA_MTP_RE25));
...
err:
	...
	if (ret) {
		...
		pr_info("Device %x calibration completed successfully\n", i2c->addr);
		return count;
	}
}
...
void tfa98xx_remove_sysfs(struct tfa98xx *tfa98xx)
{
	...
	ret = misc_deregister(&tfa98xx->calibrate_device);
	return;
}
#endif

extern void hw_set_smartpa_type(const char* smartpa_type);
