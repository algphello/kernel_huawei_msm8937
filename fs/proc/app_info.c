		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	AppStr = kmalloc(buf_len+HW_BUF_LEN_OFFSET_5, GFP_KERNEL);
	if (NULL == AppStr)
	{
	...
	memset(AppStr, 0, (buf_len + HW_BUF_LEN_OFFSET_5));
	snprintf(AppStr, (buf_len + HW_BUF_LEN_OFFSET_5 - 1), "%s", buf);
...
int app_info_set(const char * name, const char * value)
{
	...
	{
	    pr_err("%s: name or value ptr is NULL", __func__);
        return HW_FUNC_RET_FAIL;
	}
